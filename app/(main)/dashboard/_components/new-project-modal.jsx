...
export function NewProjectModal({ isOpen, onClose }) {
  ...
  const [loadingPhase, setLoadingPhase] = useState(""); // "uploading" | "creating"
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);

  const { mutate: createProject } = useConvexMutation(api.projects.create);
  const { isReady: isDemoUserReady } = useDemoUser();
  const { data: projects } = useConvexQuery(
    api.projects.getUserProjects,
    isDemoUserReady ? {} : "skip"
  );
  ...
  const handleCreateProject = async () => {
    ...
    try {
      // Phase 1: Upload to ImageKit via our API route
      ...
      // Phase 2: Create project in Convex
      ...
      while (retries > 0) {
        try {
          ...
          // Wait 1s before retry (user may not be stored in Convex yet)
